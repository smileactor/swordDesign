...
namespace Com.MyCompany.MyGame
{
    ...
    public class PlayerManager : MonoBehaviourPunCallbacks, IPunObservable
    {
        ...
        void OnTriggerEnter(Collider other)
        {
            ...
            if (other.GetComponentInParent<PhotonView>().IsMine)//这样判断其实比较耗性能，待优化
            {
                return;
            }
            Health -= 0.1f;
        ...
        void OnTriggerStay(Collider other)
        {
            ...
            Health -= 0.1f * Time.deltaTime;//利用时间平衡帧数造成的影响
        }
        #endregion
        ...
        public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
        {
            if (stream.IsWriting)//同步开火状态，不然就只有自己知道自己开火
            {
                ...
                stream.SendNext(IsFiring);//如果时自己，则发送
                stream.SendNext(this.Health);
            }
            else
            {
                // Network player, receive data
                this.IsFiring = (bool)stream.ReceiveNext();//不是自己，则接受该信息并同步
